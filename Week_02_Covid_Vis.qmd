...
```{r}
covid_data_long |> 
  filter(count_type == "Tests" & race != "Total") |> ggplot(aes(x = state, y= count)) + geom_col(aes(fill = race), position = "fill") + labs(title = "Proportion of Tests by State and Race")
```
```{R}
covid_data_long |>
  filter(race != "Total" & state %in% c("CA", "DE", "IL", "KS", "MO", "NV", "RI", "UT")) |> ggplot(aes(x = state, y = population)) + geom_col(aes(fill = race), position = "fill") + labs(State Populations by Race)
```
