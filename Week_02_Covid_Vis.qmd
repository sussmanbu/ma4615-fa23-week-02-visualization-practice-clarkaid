...
```{r}
covid_data_long |> 
  filter(count_type == "Tests" & race != "Total") |> ggplot(aes(x = state, y= count)) + geom_col(aes(fill = race), position = "fill") + geom_col(aes(x=state, y = population, fill = race), position = "fill") + labs(title = "Proportion of Tests by State and Race")
```
